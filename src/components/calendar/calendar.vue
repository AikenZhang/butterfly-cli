<template>
  <div class="m-calendar" ref="calendar">
    <div class="m-toolbar">
      <span class="m-prev-btn" @click="changeYear('prev')">
        <img :src="require('_img/arrowDb.png')">
      </span>
      <span class="m-prev-btn" @click="changeMonth('prev')">
        <img :src="require('_img/arrowSg.png')">
      </span>
      <span class="m-date">
        {{`${showDate.year}年${showDate.month}月`}}
      </span>
      <span class="m-next-btn" @click="changeMonth('next')">
        <img :src="require('_img/arrowSg.png')">
      </span>
      <span class="m-next-btn" @click="changeYear('next')">
        <img :src="require('_img/arrowDb.png')">
      </span>
    </div>
    ...
    onDayClick(day) {
      ...
      case 'during':
        if (this.selectDate.length === 0) {
          ...
          let firstDateTime = this.selectDate[0].$d.getTime()
          let secendDateTime = day.dateTime.$d.getTime()
          // if(Math.abs(Number(firstDateTime-secendDateTime)) > ((this.interval - 1) * 24*60*60*1000)){
          //   Notify({
          //     message: '超出范围，最大选择期间为7天',
          //     duration: 2000,
          //     background: '#ff0000'
          //   })
          //   return
